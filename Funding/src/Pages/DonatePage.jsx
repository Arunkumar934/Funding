...
const DonatePage = () => {
  ...
  const handleDelete = (id) => {
    axios
      .delete("http://localhost:7000/api/carts/" + id)
      .then((result) => {
        console.log(result);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mt-10 p-6">
            {filterUsers.map((cart) => (
              ...
              >
                ...
                <div className="text-gray-700 text-lg text-start px-4 leading-7 mt-5">
                  {cart.about2}{" "}
                  <Link to={`/cart/${cart._id}`} className="inline-block mt-4">
                    <span className="p-3 rounded-lg text-pink-800 bg-white hover:bg-pink-700 hover:text-white transition duration-300">
                      View More
                    </span>
                  </Link>
                  {/* <span
                    onClick={(e) => handleDelete(cart._id)}
                    className="hidden ms-5 mt-10 p-3 rounded-lg text-pink-800 bg-white hover:bg-pink-700 hover:text-white transition duration-300"
                  >
                    Delete
                  </span> */}
                </div>
                ...
                <div
                  onClick={(e) => handleDelete(cart._id)}
                  className="ms-5 mt-5 w-20 text-center p-3 rounded-lg text-pink-800 bg-white hover:bg-pink-700 hover:text-white transition duration-300"
                >
                  Delete
                </div>
